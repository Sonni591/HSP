 ...
 * the cell. The EditingCell is used in a Callback and therfore used in the
 * JavaFX TableViews
 */
public class EditingCellDoubleProperty extends TableCell<DoubleProperty, Number> {
     ...
     * @param decimals
     */
    public EditingCellDoubleProperty(Decimals decimals) {
    ...
    private void createTextField() {
        ...
        textField.focusedProperty().addListener(new ChangeListener<Boolean>() {
            ...
            public void changed(ObservableValue<? extends Boolean> arg0, Boolean arg1, Boolean arg2) {
                if (!arg2) {
                    try {
                        commitEdit(decimals.getDecimalFormat().parse(textField.getText()));
    ...
    private String getString() {
        return getItem() == null ? "0" : decimals.getDecimalFormat().format(getItem()).toString();
    }
     ...
     * @return
     */
    private TableColumn<DoubleProperty, ?> getNextColumn(boolean forward) {
        List<TableColumn<DoubleProperty, ?>> columns = new ArrayList<>();
        for (TableColumn<DoubleProperty, ?> column : getTableView().getColumns()) {
     ...
     * @return
     */
    private List<TableColumn<DoubleProperty, ?>> getLeaves(TableColumn<DoubleProperty, ?> root) {
        List<TableColumn<DoubleProperty, ?>> columns = new ArrayList<>();
        if (root.getColumns().isEmpty()) {
            ...
            for (TableColumn<DoubleProperty, ?> column : root.getColumns()) {
                columns.addAll(getLeaves(column));
            }
            return columns;
        }
